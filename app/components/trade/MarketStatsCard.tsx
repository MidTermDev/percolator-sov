...
import { formatTokenAmount, formatUsd, formatBps, formatFundingRate } from "@/lib/format";
import { useLivePrice } from "@/hooks/useLivePrice";

export const MarketStatsCard: FC = () => {
  const { engine, params, insuranceFund, fundingRate, loading } = useEngineState();
  ...
  const stats = [
    ...
    { label: "Insurance Fund", value: `${formatTokenAmount(insuranceFund?.balance ?? 0n)} ${symbol}` },
    { label: "Funding Rate", value: fundingRate !== null ? formatFundingRate(fundingRate) + "/yr" : "N/A" },
  ];
