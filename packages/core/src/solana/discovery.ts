...
/** Full slab size: header(72) + config(320) + engine accounts region */
const SLAB_DATA_SIZE = 992_560;

/**
 * We need enough bytes to parse header(72) + config(320) + engine state
 * fields up to nextAccountId (engine offset 928+8=936).
 * Total: 392 (engine start) + 936 = 1328 bytes minimum.
 * Use 1400 to have some margin.
 */
const HEADER_SLICE_LENGTH = 1400;

// Re-derive minimal engine layout constants for light parsing.
// These match the offsets in slab.ts ENGINE_* constants.
...
/**
 * Light engine parser that works with partial slab data (no accounts array).
 * Requires at least ENGINE_OFF + 936 bytes.
 */
function parseEngineLight(data: Uint8Array): EngineState {
  const base = ENGINE_OFF;
  const minLen = base + 936; // through nextAccountId
 ...
 *
 * Uses getProgramAccounts with dataSize filter + dataSlice to download only
 * the first ~1400 bytes of each ~992KB slab account.
 */
export async function discoverMarkets(
  connection: Connection,
  programId: PublicKey,
): Promise<DiscoveredMarket[]> {
  // Try dataSize filter first; fall back to memcmp on PERCOLAT magic bytes
  // (some RPCs don't support dataSize filters)
  let accounts;
  try {
  ...
  for (const { pubkey, account } of accounts) {
    ...
    // Validate PERCOLAT magic at offset 0
    let valid = true;
